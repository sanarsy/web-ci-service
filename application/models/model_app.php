 ...
 public function getkodeservice2()
  ...
  public function datakaryawan()
    ...
    public function datalist_service()
    {
        ...
        $query = $this->db->query("SELECT A.status, DATE_FORMAT(A.tanggal, '%d-%m-%Y %H:%i:%s')as tgl_service, A.skomputer, A.sparepart, A.id_kondisi, A.id_service,
									A.problem_detail, A.category, A.id_barang, D.nama, F.nama_divisi, DATE_FORMAT(A.tanggal_service, '%d-%m-%Y %H:%i:%s')as tgl_svc,
									B.nama_barang, (B.keterangan) as ket, B.jenis,
									C.nama_barang, (C.keterangan) as ket, C.jenis
									FROM service A 
									LEFT JOIN karyawan D ON D.nik = A.reported
									LEFT JOIN barang C ON C.id_barang = A.id_barang
									LEFT JOIN barang B ON B.nik = A.barang
